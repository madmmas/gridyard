...
**Batch status:** issues filed on GitHub (§19–§24). Implementation PRs not yet merged.
...
| 23 | [#47](https://github.com/madmmas/gridyard/issues/47) | requestAnimationFrame batching for repaints | (this PR) |
...
### Acceptance criteria
- [ ] Switching the demo's sample user changes what's visible/editable, using the real resolution function from batch 3, not a separate ad hoc check
- [ ] A `hidden` field never appears in the DOM/canvas paint, not just visually suppressed
- [ ] Attempting to edit a `view`-only field is rejected with a clear signal (not a silent failure)

### Testing requirements
- [ ] Vitest tests covering: hidden field isn't rendered, view-only field rejects edit, full-access field behaves as before
...
### Acceptance criteria
- [ ] The same `loans` record renders correctly as both a grid row and a form, from the same underlying binding — not two independent data paths
- [ ] Sections render as visually distinct groups, not a flat field dump
- [ ] Field permissions from issue #19 apply identically in form view

### Testing requirements
- [ ] Vitest tests for the form-layout logic (given a schema + record, produce the right section/field structure)
- [ ] `npm test --workspaces --if-present` and typecheck pass
...
### Acceptance criteria
- [ ] The Employee Management workspace renders correctly using the same engine code as Loan Review, with no Loan-Review-specific branches added to make it work
- [ ] Any genuinely Loan-Review-specific assumption found in the schema loader or adapter gets fixed, not worked around
- [ ] Switching between the two workspaces in `web-demo` works without a reload

### Testing requirements
- [ ] Vitest tests for the new workspace definition's validation, same pattern as batch 2's issue #10 tests
- [ ] `npm test --workspaces --if-present` and typecheck pass
...
### Acceptance criteria
- [ ] A grid with several thousand rows scrolls smoothly, painting only the visible window
- [ ] Scrolling to a previously off-screen row shows its correct, already-computed value immediately (no flash of stale/empty state)
- [ ] Existing selection/editing/undo behavior (batch 2/3) keeps working unchanged under virtualization

### Testing requirements
- [ ] Vitest tests for the viewport-to-visible-row-range calculation (pure function)
- [ ] `npm test --workspaces --if-present` and typecheck pass; note in the PR what row count was used to manually confirm smooth scrolling, since that's not something a unit test proves by itself
...
**Status:** done — issue [#47](https://github.com/madmmas/gridyard/issues/47)
...
### Acceptance criteria
- [ ] An edit that dirties N dependent cells triggers exactly one paint per affected region per frame, not N paints
- [ ] No stale paint — the single batched paint reflects every dirtied cell's final value, not an intermediate state
- [ ] No regression in perceived responsiveness for a single-cell edit (still feels instant)

### Testing requirements
- [ ] Vitest tests using a fake/mocked `requestAnimationFrame` to assert paint-call counts for single-edit vs. cascading-edit scenarios
- [ ] `npm test --workspaces --if-present` and typecheck pass
...
### Acceptance criteria
- [ ] Typing a search term highlights all matching cells in the active region and scrolls the first match into view
- [ ] Next/previous controls move through matches correctly, wrapping at the ends
- [ ] Clearing the search removes all highlights and doesn't leave the viewport in a broken scroll state

### Testing requirements
- [ ] Vitest tests for match-finding and next/prev navigation logic (pure functions, not DOM)
- [ ] `npm test --workspaces --if-present` and typecheck pass
